...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self addSubview:self.goBackBt];
        [self.goBackBt mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        [self.imgV mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.height.equalTo(@21);
            make.centerY.equalTo(self);
            make.left.equalTo(self.goBackBt.mas_right).offset(5);
        }];
...
-(UIButton *)goBackBt {
    if (_goBackBt == nil) {
        _goBackBt = [[UIButton alloc] init];
        [_goBackBt setBackgroundImage:[UIImage imageNamed:@"kk3"] forState:UIControlStateNormal];
    }
    return _goBackBt;
}
...
-(UIImageView *)imgV {
    if (_imgV == nil) {
        ...
        _imgV.image = [UIImage imageNamed:@"kk1"];
    }
    return _imgV;
}
