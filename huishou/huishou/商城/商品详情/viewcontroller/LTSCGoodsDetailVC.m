...
- (void)loadDetailData {
    ...
    [LTSCNetworking networkingPOST:good_detailNew parameters:dict returnClass:[LTSCGoodsDetailRootModel class] success:^(NSURLSessionDataTask *task, LTSCGoodsDetailRootModel *responseObject) {
...
- (void)addCarAction {
    ...
    [LTSCNetworking networkingPOST:add_cart parameters:dict returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([responseObject[@"key"] integerValue] == 1000) {
            ...
            if (data.intValue == 1) {
                NSString *str = [NSString stringWithFormat:@"%d", self.carNum.integerValue + 1];
                self.bottomView.numLabel.hidden = NO;
                self.bottomView.numLabel.text = str;
                if (str.length == 1) {
                    [self.bottomView.numLabel mas_updateConstraints:^(MASConstraintMaker *make) {
...
- (void)buyNowAction {
    ...
    if (self.guigeView.guigeArr.count > 0) {
        ...
        model.num = @"1";
        model.good_price = self.detailModel.normalPrice;
    }
