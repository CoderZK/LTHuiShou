...
- (void)viewDidLoad {
    ...
    CGFloat contetH = [NSString getString:content lineSpacing:2 font:[UIFont systemFontOfSize:11] width:(ScreenW - 30)];
...
- (void)sendCode {
    if (self.oneCode) {
        ...
        if ([self.headerView.phoneTF.text isEqualToString:[LxmTool ShareTool].userModel.tel]) {
 ...
 */
- (void)exitClick {
    [LTSCNetworking networkingPOST:app_logout parameters:@{@"token":[LxmTool ShareTool].session_token} returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        if ([responseObject[@"key"] integerValue] == 1000) {
