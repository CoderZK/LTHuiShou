...
- (void)loadWuLiuInfo {
    ...
    [LTSCNetworking networkingPOST:way_detail parameters:@{@"token":SESSION_TOKEN,@"id":self.orderID} returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([responseObject[@"key"] integerValue] == 1000) {
            selfWeak.dataArr = [LxmWuLiuInfoStateModel mj_objectArrayWithKeyValuesArray:responseObject[@"result"][@"list"]];
            self.mapData = [LxmWuLiuInfoMapModel mj_objectWithKeyValues:responseObject[@"result"][@"map"]];
            [selfWeak.tableView reloadData];
        } else {
            [UIAlertController showAlertWithmessage:responseObject[@"message"]];
        }
