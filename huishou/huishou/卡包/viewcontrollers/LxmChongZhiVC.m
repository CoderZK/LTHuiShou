...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        cell.textLabel.text = [NSString stringWithFormat: @"应付金额：¥%0.2f", self.moneyStr.floatValue];
        return cell;
    }
    ...
    cell.iconImgView.image = [UIImage imageNamed:indexPath.row == 0 ? @"alipay_pay" : @"wechat_pay"];
...
//    dict[@"type"] = @(self.currentIndex+1);
    ...
    NSLog(@"%@",TOKEN);
     WeakObj(self);
    [LxmNetworking networkingPOST:rechargeNew parameters:dict returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        [LxmNetworking networkingPOST:str parameters:dict returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            ...
            if ([responseObject[@"key"] integerValue] == 1000) {
                ...
                self.payDic = responseObject[@"result"];
                    ...
                    self.payDic = responseObject[@"result"][@"map"];
                    [self goWXpay];
                }
