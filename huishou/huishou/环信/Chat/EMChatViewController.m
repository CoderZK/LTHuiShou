...
//    self.titleLabel.text = self.conversationModel.name;
    self.titleLabel.text = self.toUserName;
    if (self.toUserName.length == 0) {
        self.titleLabel.text = self.conversationModel.name;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cellString length] > 0) {
        ...
        if (model.emModel.direction == EMMessageDirectionSend) {
            [cell.avatarView sd_setImageWithURL:[NSURL URLWithString:[[LxmTool ShareTool].userModel.head_pic stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet  URLQueryAllowedCharacterSet]]] placeholderImage:[UIImage imageNamed:@"user"] options:SDWebImageRetryFailed];
        }else {
             [cell.avatarView sd_setImageWithURL:[NSURL URLWithString:[self.toUserHeadPic stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet  URLQueryAllowedCharacterSet]]] placeholderImage:[UIImage imageNamed:@"user"] options:SDWebImageRetryFailed];
            cell.nameLabel.text = self.toUserName;
        }
