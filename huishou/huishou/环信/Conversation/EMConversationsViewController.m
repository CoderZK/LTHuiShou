...
/**  */
@property(nonatomic , strong)NSMutableArray<LxmUserInfoModel *> *userArr;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (LxmUserInfoModel * userModel in self.userArr) {
        if ([userModel.im_code isEqualToString:[model.emModel.conversationId uppercaseString]]) {
            [cell.avatarView sd_setImageWithURL:[NSURL URLWithString:[userModel.head_pic stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet  URLQueryAllowedCharacterSet]]] placeholderImage:[UIImage imageNamed:@"user"] options:SDWebImageRetryFailed];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (LxmUserInfoModel * uM  in self.userArr) {
        if ([[model.emModel.conversationId uppercaseString] isEqualToString:uM.im_code]) {
            chatController.toUserHeadPic = uM.head_pic;
            chatController.toUserName = uM.username;
        }
    }
...
- (void)loadIDsData {
    ...
    [LxmNetworking networkingPOST:get_user_im parameters:dict returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        
        if ([responseObject[@"key"] integerValue] == 1000) {
            
            self.userArr =[LxmUserInfoModel mj_objectArrayWithKeyValuesArray:responseObject[@"result"][@"list"]];
            [self.tableView reloadData];
