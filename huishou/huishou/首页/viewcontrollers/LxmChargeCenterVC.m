...
@interface LxmChargeCenterVC ()<CNContactPickerDelegate,MFMessageComposeViewControllerDelegate>

@property (nonatomic, strong) UIImageView *bgImgView;

@property (nonatomic, strong) LxmChargeCenterTopView *topView;
...
@implementation LxmChargeCenterVC
...
- (LxmChargeCenterTopView *)topView {
    if (!_topView) {
        _topView = [LxmChargeCenterTopView new];
        WeakObj(self);
        _topView.didMoneyClickBlock = ^(NSString *money) {
            selfWeak.moneyStr = money;
             [selfWeak.tixianButton setTitle:[NSString stringWithFormat:@"¥%.2f 立即充值", money.doubleValue] forState:UIControlStateNormal];
        };
    }
    return _topView;
}
...
//        _phoneLabel.font = [UIFont boldSystemFontOfSize:25];
//    }
//    return _phoneLabel;
//}
...
- (UIButton *)tixianButton {
    if (!_tixianButton) {
        ...
        [_tixianButton setTitle:@"¥10.00 立即充值" forState:UIControlStateNormal];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self initSubViews];
    self.phoneTF.text = LxmTool.ShareTool.userModel.telephone;
    self.textLabel.text = [NSString stringWithFormat:@"默认号码(%@)",[NSString pushSignIn:LxmTool.ShareTool.userModel.telephone]];
    self.moneyStr = @"10";
}
...
- (void)contactPicker:(CNContactPickerViewController *)picker didSelectContact:(CNContact *)contact {
    ...
    for (CNLabeledValue *labeledValue in contact.phoneNumbers){
        ...
        self.textLabel.text = [NSString stringWithFormat:@"默认号码(%@)",area];
...
- (void)chargeNowClick:(UIButton *)btn {
    ...
    LxmChongZhiVC * vc =[[LxmChongZhiVC alloc] initWithTableViewStyle:(UITableViewStyleGrouped)];
